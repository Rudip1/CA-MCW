...
------------------------------------
Record start + waypoints for TRAINING-data batch runs. No robot movement,
no planner — just RViz + pose_recorder.

Each Ctrl-C appends a new row (run_id auto-increments) to:
  <maps_dir>/<map_name>/training_goalposes_collect.csv

CSV format (wide, one tour per row):
  run_id, notes, start_x, start_y, start_yaw, g1_x, g1_y, g1_yaw, g2_x, …

Use the row's run_id as the `run_id` arg to vf_data_training_batch_fixedwt.
...
Usage:
  ros2 launch vf_robot_utils training_goalposes_collect.launch.py map_name:=house_my1_map
"""
...
def generate_launch_description():
    ...
    pose_recorder_node = Node(
        ...
        parameters=[{
            ...
            'map_name':      map_name,
            'output_dir':    maps_dir,
            'csv_filename':  'training_goalposes_collect.csv',
        }],
    )

    return LaunchDescription([
        map_name_arg, maps_dir_arg, use_sim_time_arg,
        LogInfo(msg=['[training_goalposes_collect] map: ', map_yaml,
                     '  →  training_goalposes_collect.csv']),
        map_server_node,
        lifecycle_manager,
        rviz_node,
        pose_recorder_node,
