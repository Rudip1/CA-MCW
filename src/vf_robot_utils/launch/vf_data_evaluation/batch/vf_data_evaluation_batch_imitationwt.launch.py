#!/usr/bin/env python3
"""
vf_data_evaluation_batch_imitationwt.launch.py
----------------------------------------------
Batch (CSV-driven) evaluation of one trained imitation-weights variant.
Mirrors the stack used by the working
vf_data_training_batch_fixedwt.launch.py (bringup + data_collector_node +
tour_runner), but:

  - bringup loads controller:=vf_imitationwt and imitation_weights:=<ch>_<hp>
    (the Python sidecar imitation_inference_node publishes /cmd_vel_nav,
    the C++ plugin stays in PASSIVE mode);
  - the collector's `training_root` parameter is repointed at
    EVALUATION_ROOT so HDF5s land under vf_data/vf_data_evaluation/.

Output path:
  vf_data/vf_data_evaluation/batch/<map>/<goal_xy>/<Planner>/imitationwt_<hp>_<ch>/run_*.h5
...
def generate_launch_description():
    return build_eval_batch_launch_description("imitationwt")
