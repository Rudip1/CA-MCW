...
#
# Reference (world + bundled hospital model catalogue):
#   Tommaso Vandermeer — Hospitalbot-Path-Planning
#   https://github.com/TommasoVandermeer/Hospitalbot-Path-Planning
#   ROS 2 / Gazebo / OpenAI Gym / Stable-Baselines3 RL infrastructure for
#   motion planning in an indoor hospital scene (AWS RoboMaker assets +
#   Pioneer 3AT). This launch reuses the world only; the spawned robot is
#   the UVC1 ViroFighter from vf_robot_description.
#
# Hospital Tommaso (hospital) world (SDF pipeline) — robot loaded from model.sdf in vf_robot_gazebo/models/uvc1_virofighter
#
# Node graph:
...
#
# World file:    worlds/hospital_Tommaso_world/hospital_Tommaso_hospital.world
# Env models:    models/hospital_Tommaso_models
# Robot models:  models/uvc1_common, models/uvc1_virofighter (added via base models/ path)
#
# Run:  ros2 launch vf_robot_gazebo hospital_Tommaso_hospital_world_sdf.launch.py
...
def generate_launch_description():
    ...
    declare_y_pose = DeclareLaunchArgument(
        "y_pose",
        default_value="2.0",
        description="Y spawn position (Gazebo +Y / green axis — offset to clear obstacle at origin)",
    )
    ...
    world = os.path.join(pkg_vf_gazebo, "worlds", "hospital_Tommaso_world", "hospital_Tommaso_hospital.world")
    ...
    gazebo_model_path = SetEnvironmentVariable(
        ...
        value=os.pathsep.join(
            [
                os.path.join(pkg_vf_gazebo, "models", "hospital_Tommaso_models"),  # env-specific assets
    ...
    spawn_robot_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={
            "x_pose": x_pose,
            "y_pose": y_pose,
            "z_pose": "0.2",  # extra clearance over the AWS triangulated floor mesh
            "theta": theta,
        }.items(),
    )
